...
import { FindCountryByCodeUseCase } from '../use-cases/country'
import { ImportCountryOptionsDto } from './dtos'
import {
  COUNTRY_IMPORT_QUEUE,
  CountryImportJobData
} from '@modules/lead/infra/queues'

const csvStorage = diskStorage({
  destination: tmpdir(),
  filename: (_req, file, cb) => {
    const uniqueName = `country-import-${randomUUID()}-${file.originalname}`
    cb(null, uniqueName)
  }
})

@ApiTags('Countries (Países)')
@Controller('country')
export class CountryController {
  constructor(
    @InjectQueue(COUNTRY_IMPORT_QUEUE)
    private readonly importQueue: Queue<CountryImportJobData>
  ) {}

  @Inject(FindCountryByCodeUseCase)
  private readonly findByCodeUseCase: FindCountryByCodeUseCase

  @Post('import')
  @UseInterceptors(FileInterceptor('file', { storage: csvStorage }))
  @ApiOperation({
    summary: 'Import Country data from CSV file (async)',
    description:
      'Queues an import job for Country (Países) data from a CSV file. Returns a job ID to track progress.'
  })
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        statusUrl: {
          type: 'string',
          example: '/api/country/import/status/123'
        }
      }
    }
  })
  ...
  async importFromFile(
    ...
    @Body() options: ImportCountryOptionsDto,
    @Res() res: Response
  ) {
    if (!file) {
    ...
    return res.status(HttpStatus.ACCEPTED).json({
      ...
      statusUrl: `/api/country/import/status/${job.id}`
    })
  }
  ...
  @ApiOperation({
    ...
    description: 'Returns a country by its code'
  })
  @ApiResponse({
    status: HttpStatus.OK,
