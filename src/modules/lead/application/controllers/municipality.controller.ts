...
import { FindMunicipalityByCodeUseCase } from '../use-cases/municipality'
import { ImportMunicipalityOptionsDto } from './dtos'
import {
  MUNICIPALITY_IMPORT_QUEUE,
  MunicipalityImportJobData
} from '@modules/lead/infra/queues'

const csvStorage = diskStorage({
  destination: tmpdir(),
  filename: (_req, file, cb) => {
    const uniqueName = `municipality-import-${randomUUID()}-${file.originalname}`
    cb(null, uniqueName)
  }
})

@ApiTags('Municipalities (Municípios)')
@Controller('municipality')
export class MunicipalityController {
  constructor(
    @InjectQueue(MUNICIPALITY_IMPORT_QUEUE)
    private readonly importQueue: Queue<MunicipalityImportJobData>
  ) {}

  @Inject(FindMunicipalityByCodeUseCase)
  private readonly findByCodeUseCase: FindMunicipalityByCodeUseCase

  @Post('import')
  @UseInterceptors(FileInterceptor('file', { storage: csvStorage }))
  @ApiOperation({
    summary: 'Import Municipality data from CSV file (async)',
    description:
      'Queues an import job for Municipality (Municípios) data from a CSV file. Returns a job ID to track progress.'
  })
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        statusUrl: {
          type: 'string',
          example: '/api/municipality/import/status/123'
        }
      }
    }
  })
  ...
  async importFromFile(
    ...
    @Body() options: ImportMunicipalityOptionsDto,
    @Res() res: Response
  ) {
    if (!file) {
    ...
    return res.status(HttpStatus.ACCEPTED).json({
      ...
      statusUrl: `/api/municipality/import/status/${job.id}`
    })
  }
  ...
  @Get('code/:code')
  @ApiOperation({
    summary: 'Find Municipality by code',
    description: 'Returns a municipality by its code'
  })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Municipality data retrieved successfully'
  })
