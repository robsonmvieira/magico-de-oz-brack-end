...
export const LEGAL_NATURE_IMPORT_QUEUE = 'legal-nature-import'

export interface LegalNatureImportJobData {
  filePath: string
  batchSize: number
  delimiter: string
  skipHeader: boolean
...
export interface LegalNatureImportJobResult {
...
@Processor(LEGAL_NATURE_IMPORT_QUEUE)
export class LegalNatureImportProcessor {
  private readonly logger = new Logger(LegalNatureImportProcessor.name)

  constructor(@Inject(PG_POOL) private readonly pool: Pool) {}

  @Process()
  async handleImport(
    job: Job<LegalNatureImportJobData>
  ): Promise<LegalNatureImportJobResult> {
  ...
  private async importWithCopy(
    ...
    skipHeader: boolean,
    job: Job<LegalNatureImportJobData>
  ): Promise<Omit<LegalNatureImportJobResult, 'durationMs'>> {
    const client = await this.pool.connect()

    try {
      this.logger.log('Starting COPY directly to legal_natures table...')
      const copyQuery = copyFrom(
        `COPY legal_natures (id, code, description, created_at, updated_at, is_deleted, is_active, is_blocked) FROM STDIN WITH (FORMAT csv, DELIMITER ',', NULL '')`
      )
  ...
  private transformLine(line: string, delimiter: string): string | null {
    ...
    // Example: "1023";"Órgão Público do Poder Executivo Estadual ou do Distrito Federal"
  ...
  onCompleted(
    job: Job<LegalNatureImportJobData>,
    result: LegalNatureImportJobResult
  ) {
    this.logger.log(
  ...
  @OnQueueFailed()
  onFailed(job: Job<LegalNatureImportJobData>, error: Error) {
