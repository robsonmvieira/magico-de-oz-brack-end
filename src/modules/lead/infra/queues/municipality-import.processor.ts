...
export const MUNICIPALITY_IMPORT_QUEUE = 'municipality-import'

export interface MunicipalityImportJobData {
  filePath: string
  batchSize: number
  delimiter: string
  skipHeader: boolean
...
export interface MunicipalityImportJobResult {
...
@Processor(MUNICIPALITY_IMPORT_QUEUE)
export class MunicipalityImportProcessor {
  private readonly logger = new Logger(MunicipalityImportProcessor.name)

  constructor(@Inject(PG_POOL) private readonly pool: Pool) {}

  @Process()
  async handleImport(
    job: Job<MunicipalityImportJobData>
  ): Promise<MunicipalityImportJobResult> {
  ...
  private async importWithCopy(
    ...
    skipHeader: boolean,
    job: Job<MunicipalityImportJobData>
  ): Promise<Omit<MunicipalityImportJobResult, 'durationMs'>> {
    const client = await this.pool.connect()

    try {
      this.logger.log('Starting COPY directly to municipalities table...')
      const copyQuery = copyFrom(
        `COPY municipalities (id, code, name, created_at, updated_at, is_deleted, is_active, is_blocked) FROM STDIN WITH (FORMAT csv, DELIMITER ',', NULL '')`
      )
  ...
  private transformLine(line: string, delimiter: string): string | null {
    ...
    // Example: "0128";"BOM JESUS DAS SELVAS"
  ...
  onCompleted(
    job: Job<MunicipalityImportJobData>,
    result: MunicipalityImportJobResult
  ) {
    this.logger.log(
  ...
  @OnQueueFailed()
  onFailed(job: Job<MunicipalityImportJobData>, error: Error) {
