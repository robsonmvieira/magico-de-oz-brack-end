...
export const CNAE_IMPORT_QUEUE = 'cnae-import'

export interface CnaeImportJobData {
  filePath: string
  batchSize: number
  delimiter: string
  skipHeader: boolean
...
export interface CnaeImportJobResult {
...
@Processor(CNAE_IMPORT_QUEUE)
export class CnaeImportProcessor {
  private readonly logger = new Logger(CnaeImportProcessor.name)

  constructor(@Inject(PG_POOL) private readonly pool: Pool) {}

  @Process()
  async handleImport(
    job: Job<CnaeImportJobData>
  ): Promise<CnaeImportJobResult> {
  ...
  private async importWithCopy(
    ...
    skipHeader: boolean,
    job: Job<CnaeImportJobData>
  ): Promise<Omit<CnaeImportJobResult, 'durationMs'>> {
    const client = await this.pool.connect()

    try {
      this.logger.log('Starting COPY directly to cnaes table...')
      const copyQuery = copyFrom(
        `COPY cnaes (id, code, description, created_at, updated_at, is_deleted, is_active, is_blocked) FROM STDIN WITH (FORMAT csv, DELIMITER ',', NULL '')`
      )
  ...
  private transformLine(line: string, delimiter: string): string | null {
    ...
    // Example: "0112199";"Cultivo de outras fibras de lavoura temporária não especificadas anteriormente"
  ...
  @OnQueueCompleted()
  onCompleted(job: Job<CnaeImportJobData>, result: CnaeImportJobResult) {
    this.logger.log(
  ...
  @OnQueueFailed()
  onFailed(job: Job<CnaeImportJobData>, error: Error) {
