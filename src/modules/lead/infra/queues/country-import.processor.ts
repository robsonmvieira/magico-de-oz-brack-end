...
export const COUNTRY_IMPORT_QUEUE = 'country-import'

export interface CountryImportJobData {
  filePath: string
  batchSize: number
  delimiter: string
  skipHeader: boolean
...
export interface CountryImportJobResult {
...
@Processor(COUNTRY_IMPORT_QUEUE)
export class CountryImportProcessor {
  private readonly logger = new Logger(CountryImportProcessor.name)

  constructor(@Inject(PG_POOL) private readonly pool: Pool) {}

  @Process()
  async handleImport(
    job: Job<CountryImportJobData>
  ): Promise<CountryImportJobResult> {
  ...
  private async importWithCopy(
    ...
    skipHeader: boolean,
    job: Job<CountryImportJobData>
  ): Promise<Omit<CountryImportJobResult, 'durationMs'>> {
    const client = await this.pool.connect()

    try {
      this.logger.log('Starting COPY directly to countries table...')
      const copyQuery = copyFrom(
        `COPY countries (id, code, name, created_at, updated_at, is_deleted, is_active, is_blocked) FROM STDIN WITH (FORMAT csv, DELIMITER ',', NULL '')`
      )
  ...
  @OnQueueCompleted()
  onCompleted(job: Job<CountryImportJobData>, result: CountryImportJobResult) {
    this.logger.log(
  ...
  @OnQueueFailed()
  onFailed(job: Job<CountryImportJobData>, error: Error) {
