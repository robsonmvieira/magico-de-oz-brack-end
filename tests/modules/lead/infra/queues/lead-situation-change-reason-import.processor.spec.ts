import { LeadSituationChangeReasonImportProcessor } from '@modules/lead/infra/queues/lead-situation-change-reason-import.processor'
...
describe('LeadSituationChangeReasonImportProcessor', () => {
  let processor: LeadSituationChangeReasonImportProcessor
  ...
  beforeAll(async () => {
    tempDir = join(
      tmpdir(),
      `lead-situation-change-reason-import-test-${randomUUID()}`
    )
  ...
  beforeEach(async () => {
    ...
    processor = new LeadSituationChangeReasonImportProcessor(mockPool)
  })
  ...
  describe('transformLine', () => {
    ...
    it('should handle unquoted CSV fields', () => {
      const line = '01;EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA'
      const result = (processor as any).transformLine(line, ';')

      expect(result).not.toBeNull()
      const parts = result.split(',')
      expect(parts[1]).toBe('01')
      expect(parts[2]).toBe('EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA')
    })
    ...
    it('should return null for lines without code', () => {
      const line = ';DESCRICAO DO MOTIVO'
      const result = (processor as any).transformLine(line, ';')
    ...
    it('should handle different reason codes', () => {
      const testCases = [
        {
          line: '"01";"EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA"',
          code: '01',
          description: 'EXTINCAO POR ENCERRAMENTO LIQUIDACAO VOLUNTARIA'
        },
        {
          line: '"63";"OMISSAO DE DECLARACOES"',
          code: '63',
          description: 'OMISSAO DE DECLARACOES'
        },
        {
          line: '"80";"ANULACAO POR MULTICIPLIDADE"',
          code: '80',
          description: 'ANULACAO POR MULTICIPLIDADE'
        }
      ]
  ...
  describe('CSV column order mapping', () => {
    it('should correctly map Receita Federal CSV format for situation change reasons', () => {
      ...
      expect(parts[1]).toBe('33') // CODE -> code
      expect(parts[2]).toBe(
        'TRANSFERENCIA DO ORGAO LOCAL A CONDICAO DE FILIAL DO ORGAO REGIONAL'
  ...
  describe('createTransformStream', () => {
    ...
    it('should report progress every 100000 records', async () => {
      ...
      const lines = Array(5)
        .fill(null)
        .map((_, i) => `"${String(i + 1).padStart(2, '0')}";"MOTIVO ${i + 1}"`)
