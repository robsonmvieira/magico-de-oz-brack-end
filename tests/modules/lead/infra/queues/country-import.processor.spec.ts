...
describe('CountryImportProcessor', () => {
  ...
  beforeAll(async () => {
    tempDir = join(tmpdir(), `country-import-test-${randomUUID()}`)
  ...
  beforeEach(async () => {
    ...
    processor = new CountryImportProcessor(mockPool)
  })
  ...
  describe('transformLine', () => {
    it('should correctly parse CSV line with code and name', () => {
      const line = '"043";"ANTIGUA E BARBUDA"'
      ...
      // code
      expect(parts[1]).toBe('043')
      // name
      expect(parts[2]).toBe('ANTIGUA E BARBUDA')
    })
    ...
    it('should return null for lines without name', () => {
      const line = '105;'
      const result = (processor as any).transformLine(line, ';')
    ...
    it('should handle different country codes', () => {
      const testCases = [
        { line: '"105";"BRASIL"', code: '105', name: 'BRASIL' },
        { line: '"132";"ALEMANHA"', code: '132', name: 'ALEMANHA' },
        { line: '"249";"ESTADOS UNIDOS"', code: '249', name: 'ESTADOS UNIDOS' }
      ]

      for (const { line, code, name } of testCases) {
        const result = (processor as any).transformLine(line, ';')
        expect(result).not.toBeNull()
        const parts = result.split(',')
  ...
  describe('handleImport', () => {
    it('should delete temp file after successful import', async () => {
      const csvContent = createCsvContent(['"043";"ANTIGUA E BARBUDA"'])
  ...
  describe('CSV column order mapping', () => {
    it('should correctly map Receita Federal CSV format for countries', () => {
      // Format: CODE;NAME
      const line = '"043";"ANTIGUA E BARBUDA"'
      const result = (processor as any).transformLine(line, ';')
      ...
      expect(parts[1]).toBe('043') // CODE -> code
      expect(parts[2]).toBe('ANTIGUA E BARBUDA') // NAME -> name
    })
  })

  describe('createTransformStream', () => {
    it('should skip header line when skipHeader is true', async () => {
      const csvContent = createCsvContent(['"043";"ANTIGUA E BARBUDA"'], true)
      await writeFile(testFilePath, csvContent, 'utf-8')
    ...
    it('should not skip header line when skipHeader is false', async () => {
      const csvContent = createCsvContent(['"043";"ANTIGUA E BARBUDA"'], false)
      await writeFile(testFilePath, csvContent, 'utf-8')
    ...
    it('should skip empty lines', async () => {
      const csvContent = '"043";"ANTIGUA E BARBUDA"\n\n\n"105";"BRASIL"'
      await writeFile(testFilePath, csvContent, 'utf-8')
    ...
    it('should report progress every 100000 records', async () => {
      ...
      const lines = Array(5)
        .fill(null)
        .map((_, i) => `"${String(i + 1).padStart(3, '0')}";"COUNTRY ${i + 1}"`)
