...
describe('MunicipalityImportProcessor', () => {
  ...
  beforeAll(async () => {
    tempDir = join(tmpdir(), `municipality-import-test-${randomUUID()}`)
  ...
  beforeEach(async () => {
    ...
    processor = new MunicipalityImportProcessor(mockPool)
  })
  ...
  describe('transformLine', () => {
    it('should correctly parse CSV line with code and name', () => {
      const line = '"0128";"BOM JESUS DAS SELVAS"'
      ...
      // code
      expect(parts[1]).toBe('0128')
      // name
      expect(parts[2]).toBe('BOM JESUS DAS SELVAS')
    })
    ...
    it('should handle different municipality codes', () => {
      const testCases = [
        {
          line: '"0128";"BOM JESUS DAS SELVAS"',
          code: '0128',
          name: 'BOM JESUS DAS SELVAS'
        },
        { line: '"9999";"SAO PAULO"', code: '9999', name: 'SAO PAULO' },
        {
          line: '"0001";"RIO DE JANEIRO"',
          code: '0001',
          name: 'RIO DE JANEIRO'
        }
      ]
  ...
  describe('handleImport', () => {
    it('should delete temp file after successful import', async () => {
      const csvContent = createCsvContent(['"0128";"BOM JESUS DAS SELVAS"'])
  ...
  describe('CSV column order mapping', () => {
    it('should correctly map Receita Federal CSV format for municipalities', () => {
      // Format: CODE;NAME
      const line = '"0128";"BOM JESUS DAS SELVAS"'
      const result = (processor as any).transformLine(line, ';')
      ...
      expect(parts[1]).toBe('0128') // CODE -> code
      expect(parts[2]).toBe('BOM JESUS DAS SELVAS') // NAME -> name
    })
  })
  ...
  describe('createTransformStream', () => {
    ...
    it('should skip empty lines', async () => {
      const csvContent = '"0128";"BOM JESUS DAS SELVAS"\n\n\n"9999";"SAO PAULO"'
      await writeFile(testFilePath, csvContent, 'utf-8')
    ...
    it('should report progress every 100000 records', async () => {
      ...
      const lines = Array(5)
        ...
        .map(
          (_, i) =>
            `"${String(i + 1).padStart(4, '0')}";"MUNICIPALITY ${i + 1}"`
        )
