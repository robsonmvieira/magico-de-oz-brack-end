...
describe('CnaeImportProcessor', () => {
  ...
  beforeAll(async () => {
    tempDir = join(tmpdir(), `cnae-import-test-${randomUUID()}`)
  ...
  beforeEach(async () => {
    ...
    processor = new CnaeImportProcessor(mockPool)
  })
  ...
  describe('transformLine', () => {
    ...
    it('should handle unquoted CSV fields', () => {
      const line = '0111301;Cultivo de arroz'
      const result = (processor as any).transformLine(line, ';')

      expect(result).not.toBeNull()
      const parts = result.split(',')
      expect(parts[1]).toBe('0111301')
      expect(parts[2]).toBe('Cultivo de arroz')
    })
    ...
    it('should handle different CNAE codes', () => {
      const testCases = [
        {
          line: '"0111301";"Cultivo de arroz"',
          code: '0111301',
          description: 'Cultivo de arroz'
        },
        {
          line: '"4711301";"Comércio varejista de mercadorias em geral"',
          code: '4711301',
          description: 'Comércio varejista de mercadorias em geral'
        },
        {
          line: '"6201501";"Desenvolvimento de programas de computador sob encomenda"',
          code: '6201501',
          description:
  ...
  describe('handleImport', () => {
    it('should delete temp file after successful import', async () => {
      const csvContent = createCsvContent(['"0111301";"Cultivo de arroz"'])
  ...
  describe('CSV column order mapping', () => {
    it('should correctly map Receita Federal CSV format for CNAEs', () => {
      ...
      expect(parts[1]).toBe('0112199') // CODE -> code
      expect(parts[2]).toBe(
        'Cultivo de outras fibras de lavoura temporária não especificadas anteriormente'
  ...
  describe('createTransformStream', () => {
    ...
    it('should report progress every 100000 records', async () => {
      ...
      const lines = Array(5)
        ...
        .map(
          (_, i) =>
            `"${String(i + 1000000).padStart(7, '0')}";"CNAE Test ${i + 1}"`
        )
